...
We will use Docker Desktop as the container runtime for running Kubernetes workloads locally.
...
We will install [Minikube](https://minikube.sigs.k8s.io/docs/start/?arch=%2Fwindows%2Fx86-64%2Fstable%2F.exe+download) and use it to create a local Kubernetes cluster.
...
1. Argo CD is not a single pod.
2. Application Controller is the core component.
3. Repo Server interacts with Git.
4. Redis provides caching.
5. Dex enables enterprise authentication.
6. ApplicationSet automates large-scale deployments.
7. Dedicated namespaces improve management.
8. Understanding architecture simplifies troubleshooting.
9. GitOps depends on continuous reconciliation.
...
**Answer:** A controller that automatically generates multiple Argo CD Applications.
...
**Answer:** Application Controller because it maintains GitOps reconciliation.

#### 9. What happens if Git and Kubernetes states differ?

**Answer:** Argo CD marks the application OutOfSync and can automatically reconcile it.
